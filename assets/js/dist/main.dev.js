...
$(document).ready(function () {
  ...
  function OnloadAni() {
    ...
    var HeroTl = gsap.timeline(); // gsap.set(HeroTitle, {y:'100%', rotation:2, autoAlpha:0}); 
  ...
  OnloadAni(); //WE TRANSFORM ANIM//

  var text1 = $("#we-transform");
  var text2 = $("#into-content");
  var weTrans = new TimelineMax().to(text1, {
    color: "#ffffff"
  }).to(text2, {
    color: "#55fd08"
  }, 0);
  ...
  }).setPin("#pin-text").setTween(weTrans).addTo(ctrl);
  /*==================
  ---- PARALLAX ANI---
  ==================*/

  $('.hero-area').each(function () {
    var bg = $(this).find('.hero-bg'); // Add tweenmax for backgroundParallax
    ...
    var parallaxScene = new ScrollMagic.Scene({
      ...
      triggerHook: 0,
      duration: '100%'
    }).setTween(parallax).addTo(ctrl);
  });
  $('.parallax-box').each(function () {
    var bg = $(this).find('.parallax-bg'); // Add tweenmax for backgroundParallax
  ...
  $(".ani-arrowdown-wrap").each(function () {
    ...
    var LineTl = TweenMax.to(Line, 0.5, {
      y: "0%",
      ease: Linear.easeNone
    });
    new ScrollMagic.Scene({
  ...
  $(".camera-wrap").each(function () {
    ...
    var cameraTl = TweenMax.to(camera, 0.5, {
      rotation: 0,
      ease: Linear.easeNone
    });
    new ScrollMagic.Scene({
  ...
  $(".photo-gallery-wrapper").each(function () {
    ...
    var PhotoTl = TweenMax.to(Photo, 0.5, {
      y: 0,
      x: 0,
      ease: Linear.easeNone
    });
  ...
  if ($(window).width() < 900) {
    ...
    var cards = new TimelineMax().to("#card-1", {
      y: "40%"
    }).to("#card-2", {
      y: "-40%"
    }, 0);
    ...
    var cards = new TimelineMax().to("#card-1, #card-2", {
      x: 0
    });
  }
  ...
  new ScrollMagic.Scene({
    ...
    triggerHook: 1,
    duration: '60%'
  }).setTween(cards).addTo(ctrl); //get in touch anim//
  ...
  new ScrollMagic.Scene({
    ...
    triggerHook: .3,
    duration: "100%"
  }).setPin(".contact-section .section-title").addTo(ctrl);
  var arrowGetIn = $(".arrow-get-in");
  var arrowRight = TweenMax.to(arrowGetIn, {
